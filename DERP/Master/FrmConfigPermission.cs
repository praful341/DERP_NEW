...
namespace DERP.Master
{
    public partial class FrmConfigPermission : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void FrmConfigPermission_Load(object sender, EventArgs e)
        {
            try
            {
                Global.LOOKUPCopy_User(lueCopyUser);
                GetData();
                txtUName.Focus();
                FillControl();
            }
        ...
        {
            Config_PermissionMasterProperty configPermissionProperty = new Config_PermissionMasterProperty();
            ConfigPermission objConfig = new ConfigPermission();
            if (txtUName.Text.ToString() == "")
            {
                Global.Message("Please Select User Name");
                return;
            }
            if (lueCopyUser.Text.Length == 0)
            {
                Global.Message("Please Select Copy User Name");
                lueCopyUser.Focus();
                lueCopyUser.ShowPopup();
                return;
            }

            configPermissionProperty.type = "Delete";
            configPermissionProperty.user_id = Val.ToInt32(lueCopyUser.EditValue);
            m_IntRes = objConfig.Delete(configPermissionProperty);

            configPermissionProperty.type = "Copy_User";
            configPermissionProperty.user_id = Val.ToInt32(txtUName.Tag);
            configPermissionProperty.copy_user_id = Val.ToInt32(lueCopyUser.EditValue);
            m_IntRes = objConfig.Save_Copy_User(configPermissionProperty);

            if (m_IntRes == -1)
            {
                Global.Confirm("Error In Save Copy User Rights");
                txtUName.Focus();
            }
            else
            {
                if (Val.ToInt(lblMode.Tag) == 0)
                {
                    Global.Confirm("Copy User Rights Data Save Successfully");
                    GetData();
                    btnClear_Click(sender, e);
                }
                else
                {
                    Global.Confirm("Copy User Rights Data Update Successfully");
                    GetData();
                    btnClear_Click(sender, e);
                }
            }
