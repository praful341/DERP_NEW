    ...
    public partial class MDIMain : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        ...
        private void MDIMain_Shown(object sender, EventArgs e)
        {
            try
            {
                ...
                if (GlobalDec.gEmployeeProperty.user_name == "MAYUR" || GlobalDec.gEmployeeProperty.user_name == "MILAN")
                {
                    Sales_Chart();
                }
            }
        ...
        public void Sales_Chart()
        {
            ...
            try
            {
                ...
                //DataRow[] _drs = DTForm.Select("form_id = " + "1097");
                //ref_form_name = _drs[0]["REF_FORM_NAME"].ToString();

                Form k = Application.OpenForms["FRMSALESDASHBOARD"];
                ...
                if (k == null)
                {
                    ...
                    foreach (Type type in frmAssembly.GetTypes())
                    {
                        ...
                        if (type.BaseType == typeof(DevExpress.XtraEditors.XtraForm))
                        {
                            if (type.Name.ToString().ToUpper() == "FRMSALESDASHBOARD")
                            {
                                XtraForm frmShow = (XtraForm)frmAssembly.CreateInstance(type.ToString());
                                frmShow.MdiParent = this;
                                if (ref_form_name.Length > 0)
                                    frmShow.Name = ref_form_name.ToUpper();
                                if (DTForm.AsEnumerable().Where(m => m.Field<string>("form_name") == "FRMSALESDASHBOARD" && m.Field<string>("caption") == "Sales DashBoard").CopyToDataTable().Rows[0]["param"].ToString().Length > 0)
                                {
                                    string param = DTForm.AsEnumerable().Where(m => m.Field<string>("form_name") == "FRMSALESDASHBOARD" && m.Field<string>("caption") == "Sales DashBoard").CopyToDataTable().Rows[0]["param"].ToString();
                                    object[] obj1 = param.Split(',');
                                    frmShow.GetType().GetMethod("ShowForm").Invoke(frmShow, obj1);
                                    Flag = true;
                                }
        ...
        private void MDIMain_FormClosing(object sender, FormClosingEventArgs e)
        {
            try
            {
                ...
                {
                    if (Global.Confirm("Are You Sure You Want To Close ? ", "DERP", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == System.Windows.Forms.DialogResult.Yes)
                    {
                        e.Cancel = false;
                    }
                    else
        ...
        private void barBranch_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            if (FrmDevExpPopupGrid.DRow != null)
            {
                ...
                GlobalDec.gEmployeeProperty.branch_name = Val.ToString(Val.ToString(FrmDevExpPopupGrid.DRow["branch_name"]));
        ...
        private void barComp_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            if (FrmDevExpPopupGrid.DRow != null)
            {
                ...
                GlobalDec.gEmployeeProperty.company_name = Val.ToString(Val.ToString(FrmDevExpPopupGrid.DRow["Company_Name"]));
        ...
        private void BarDepartment_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            if (FrmDevExpPopupGrid.DRow != null)
            {
                ...
                GlobalDec.gEmployeeProperty.department_name = Val.ToString(Val.ToString(FrmDevExpPopupGrid.DRow["department_name"]));
        ...
        private void BarLocation_ItemClick(object sender, ItemClickEventArgs e)
        {
            ...
            if (FrmDevExpPopupGrid.DRow != null)
            {
                ...
                GlobalDec.gEmployeeProperty.location_name = Val.ToString(Val.ToString(FrmDevExpPopupGrid.DRow["location_name"]));
        ...
        private void barBtnRefreshPref_ItemClick(object sender, ItemClickEventArgs e)
        {
            try
            {
                BLL.FormPer ObjPer = new BLL.FormPer();
                ObjPer.FormName = this.Name.ToUpper();
                int IntReso = new Login().LockDelete(3, 0, ""); //For Unlock Current Barcode              
        ...
        private void MDIMain_KeyDown(object sender, KeyEventArgs e)
        {
            ...
            {
                if (GlobalDec.gEmployeeProperty.Allow_Developer == 0 && ObjPer.CheckFormPass(mStrPassword) == true)
                {
                    mStrPassword = "";
                    GlobalDec.gEmployeeProperty.Allow_Developer = 1;
                ...
                //else if (GlobalDec.gEmployeeProperty.Allow_Developer == 1 && ObjPer.CheckFormPass(mStrPassword) == true)
                //{
                //    mStrPassword = "";
            ...
            //mStrPassword = mStrPassword + Global.KeycodeToChar(e);

            //if (e.KeyCode == Keys.B && e.Modifiers == (Keys.Control))
            //{
            //    mStrPassword = "";

            //    //if (GlobalDec.gEmployeeProperty.DB_Allow_Developer == 1 && ObjPer.FormPassWord != "")
            //    //{
            //    //    Global.Message("Welcome\n\nType Password & at last press {ENTER} Key....");
            //    //    //mBoolFlag = true;
            //    //}
            //}
        }
        //public void SetApplicationMenuBasedOnPart()
        //{
        //    if (GlobalDec.gEmployeeProperty.Allow_Developer == 1)
        //    {
        //        applicationMenu1.Ribbon.ApplicationIcon = Properties.Resources.Ruby_32;
        //    }
        //    else
        //    {
        //        applicationMenu1.Ribbon.ApplicationIcon = Properties.Resources.Ruby_32B;
        //    }
        //}
    }
}
