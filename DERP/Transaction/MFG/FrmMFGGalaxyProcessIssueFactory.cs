...
{
    public partial class FrmMFGGalaxyProcessIssueFactory : DevExpress.XtraEditors.XtraForm
    {
        #region Data Member
        ...
        #region Constructor
        public FrmMFGGalaxyProcessIssueFactory()
        {
            InitializeComponent();
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (Main_Lot_ID == "")
                {
                    ...
                    {
                        if (GlobalDec.gEmployeeProperty.role_name == "SURAT 4P" || GlobalDec.gEmployeeProperty.role_name == "SURAT 4P OK" || GlobalDec.gEmployeeProperty.role_name == "SURAT SARIN" || GlobalDec.gEmployeeProperty.role_name == "SURAT SARIN ENTRY" || GlobalDec.gEmployeeProperty.role_name == "SURAT 4P ENTRY" || GlobalDec.gEmployeeProperty.role_name == "SURAT RUSSIAN" || GlobalDec.gEmployeeProperty.role_name == "SURAT POLISH REPARING" || GlobalDec.gEmployeeProperty.role_name == "SURAT KATARGAM" || GlobalDec.gEmployeeProperty.role_name == "SURAT KAMALA" || GlobalDec.gEmployeeProperty.role_name == "KAMALA ADMIN")
                        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    foreach (DataRow drw in m_dtbIssueProcess.Rows)
                    {
                        ...
                        objMFGProcessIssueProperty.manager_id = Val.ToInt(lueManager.EditValue);
                        objMFGProcessIssueProperty.employee_id = Val.ToInt(lueEmployee.EditValue);
        ...
        private void backgroundWorker_ProcessIssue_RunWorkerCompleted(object sender, System.ComponentModel.RunWorkerCompletedEventArgs e)
        {
            try
            {
                ...
                if (Issue_IntRes > 0)
                {
                    Global.Confirm("Galaxy Process Issue Data Save Succesfully");
                    ClearDetails();
                }
                else
                {
                    Global.Confirm("Error In Galaxy Process Issue");
                    btnSave.Enabled = true;
        ...
        private bool GenerateProcessDetails()
        {
            ...
            try
            {
                ...
                m_dtbIssueProcess.Columns.Add("main_lot_id", typeof(int));
        ...
        private void txtMainLotID_Validated(object sender, EventArgs e)
        {
            try
            {
                if (m_dtbIssueProcess.Rows.Count > 0)
                {
                    ...
                    dtDistinct = m_dtbIssueProcess.DefaultView.ToTable(true, "main_lot_id");
                ...
                if (Save_Validate_MainLotID())
                {
                    ...
                    if (IsIssue == 1)
                    {
                        Global.Message("Lot ID Already Issued.");
                        txtMainLotID.Focus();
                        return;
                    }
                    DataTable dtTemp = new DataTable();
                    dtTemp = Global.GetRoughJangedMainStockWise(Val.ToInt(lueKapan.EditValue), Val.ToInt64(txtMainLotID.Text), Val.ToString("GALAXY_ISSUE"));
                    m_isMainLot = 1;
                    m_dtbIssueProcess = m_dtbIssueProcess.AsEnumerable().Union(dtTemp.AsEnumerable()).CopyToDataTable();
                    if (m_dtbIssueProcess.Rows.Count > 0)
                    {
                    }
                    else
                    {
                        Global.Message("Lot ID Not In A Stock");
                        txtLotID.Text = "";
                        txtLotID.Focus();
                        return;
                    }
