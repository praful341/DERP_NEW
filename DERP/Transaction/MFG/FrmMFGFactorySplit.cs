    ...
    public partial class FrmMFGFactorySplit : DevExpress.XtraEditors.XtraForm
    {
        ...
        private void backgroundWorker_LotSplit_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
        {
            try
            {
                ...
                try
                {
                    ...
                    //dtBarcodePrint.Columns.Add("lot_srno", typeof(string));
                    int count = 0;
                    int srno_count = 0;
                    ...
                    foreach (DataRow drw in m_dtbLotMixSplit.Rows)
                    {
                        ...
                        //if(m_dtbLotMixSplit.Rows.Count == Flag)
                        //{
                        //    objMFGFactorySplitProperty.flag = 1;
                        //}
                        ...
                        drw["lot_srno"] = Val.ToString(objMFGFactorySplitProperty.sub_lot_no);
                        ...
                        //drwNew["lot_srno"] = Val.ToDecimal(drw["lot_srno"]);
                        dtBarcodePrint.Rows.Add(drwNew);
                        count++;

                    }
                    if (Val.ToString(GlobalDec.gEmployeeProperty.department_name) == "GALAXY")
                    {

                        DateTime date_month = Convert.ToDateTime(dtpReceiveDate.Text);
                        string text_days = date_month.ToString("dd");
                        string text_month = date_month.ToString("MM");
                        int counts = m_dtbLotMixSplit.Rows.Count;
                        ...
                        XmlTextWriter xmlWriter = new XmlTextWriter(newValue + m_dtbLotMixSplit.Rows[0]["kapan_no"].ToString() + "(" + text_days + "-" + text_month + ")(" + counts + "-" + Val.ToString(txtLotID.Text) + ".xml", Encoding.UTF8);
                        ...
                        foreach (DataRow DR in m_dtbLotMixSplit.Rows)
                        {
                            ...
                            DateTime dt = Convert.ToDateTime(dtpReceiveDate.Text);
                            ...
                            xmlWriter.WriteStartAttribute("stone_name");//Attribute "Age"
                                                                        ...
                                                                        //string LotSrno = dtBarcodePrint.Select("lot_srno ='" + Val.ToInt(m_update_srno) + "'");
                            xmlWriter.WriteString(Val.ToString(first_cut_no + cut_no[1] + last_cut_no) + "-" + Val.ToString(DR["lot_srno"]));//Attribute Value
                            xmlWriter.WriteEndAttribute();
                            ...
                            xmlWriter.WriteString(Val.ToString(parts[0] + "-" + IntRes + "(" + text + ")" + counts)); //Attribute Value 
                            xmlWriter.WriteEndAttribute();
        ...
        private bool GenerateLotSplitDetails()
        {
            ...
            try
            {
                ...
                m_dtbLotMixSplit.Columns.Add("lot_srno", typeof(string));
