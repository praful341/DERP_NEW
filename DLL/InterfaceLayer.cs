...
namespace DLL
{
    public class InterfaceLayer
    {
        ...
        public int FindNewID_Search(string pStrConnectionString, string DBProviderName, string pStrTableName, string pStrField, string pStrCriteria)
        {
            int IntRes = 0;
            OpeSql = new OperationSql();
            IntRes = OpeSql.FindNewID_Search(pStrConnectionString, pStrTableName, pStrField, pStrCriteria);
            OpeSql = null;
            return IntRes;
        }
        ...
        public Int64 FindNewLotNo(string pStrConnectionString, string DBProviderName, string pStrTableName, string pStrField, string pStrCriteria)
        {
            Int64 IntRes = 0;
            OpeSql = new OperationSql();
            IntRes = OpeSql.FindNewLotNo(pStrConnectionString, pStrTableName, pStrField, pStrCriteria);
            OpeSql = null;
            return IntRes;
        }
        #endregion
        ...
        public int ExecuteNonQuery_History(string pStrConnectionString, string DBProviderName, Request pRequest, GlobalDec.EnumTran pEnumTran = GlobalDec.EnumTran.WithCommit)
        {
            try
            {
                int IntResult = 0;
                if (OpeSql == null) OpeSql = new OperationSql();
                IntResult = OpeSql.ExecuteNonQuery_History(pStrConnectionString, pRequest.CommandText, pRequest.CommandType, GetSqlParams(pRequest), pEnumTran);
                if (pEnumTran == GlobalDec.EnumTran.Stop || pEnumTran == GlobalDec.EnumTran.WithCommit || IntResult == -1)
                {
                    OpeSql = null;
                }
