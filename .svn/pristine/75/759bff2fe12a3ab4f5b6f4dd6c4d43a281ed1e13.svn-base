...
{
    public partial class FrmMFGBoilIssuePending_New : DevExpress.XtraEditors.XtraForm
    {
        #region Declaration
        ...
        #region Constructor
        public FrmMFGBoilIssuePending_New()
        {
            InitializeComponent();
        }
        ...
        private void FrmJangedConfirm_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                item4.FieldName = "invoice_no";
                item4.SummaryType = DevExpress.Data.SummaryItemType.Count;
                item4.ShowInGroupColumnFooter = GrdDet.Columns["invoice_no"];
        ...
        private void BtnOk_Click(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < GrdDet.DataRowCount; i++)
            {
                if (GrdDet.GetRowCellValue(i, "SEL").ToString().ToUpper() == "TRUE")
                {
                    ...
                    {
                        StrLotList = StrLotList + "," + GrdDet.GetRowCellValue(i, "purchase_detail_id").ToString();
                    }
                    else
                    {
                        StrLotList = GrdDet.GetRowCellValue(i, "purchase_detail_id").ToString();
                    }
                }
            }
            ...
            if (FormName == "FrmMFGBoilIssue_New")
            {
                if (StrLotList.Length > 0)
                {
                    DataTable DTab_Select = DTab.Select("purchase_detail_id in(" + StrLotList + ")").CopyToDataTable();
                    this.Close();
                    FrmMFGBoilIssue.GetStockData(DTab_Select);
                }
                else
                {
