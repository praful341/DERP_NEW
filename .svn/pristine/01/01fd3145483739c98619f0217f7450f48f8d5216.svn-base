...
{
    public partial class FrmMixSplit : DevExpress.XtraEditors.XtraForm
    {
        #region Data Member
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            //if (DTab_Mix.Rows.Count > 0)
            //{
            //    decimal p_numFromStockRate = 0;
            ...
            //            {
            //                p_numFromStockRate = Val.ToDecimal(objBranch.GetLetestPrice(Val.ToInt(DTab_Mix.Rows[i]["assort_id"]), Val.ToInt(DTab_Mix.Rows[i]["sieve_id"])));

            //                if (Val.ToDecimal(p_numFromStockRate) == 0)
            //                {
            //                    Global.Message("Mix Data Rate Not Found...Please Check..");
            //                    IntRes = 0;
            //                    btnSave.Enabled = true;
            //                    return;
            //                }
            ...
            //            {
            //                p_numFromStockRate = Val.ToDecimal(objBranch.GetLetestPrice(Val.ToInt(DTab_Split.Rows[i]["assort_id"]), Val.ToInt(DTab_Split.Rows[i]["sieve_id"])));

            //                if (Val.ToDecimal(p_numFromStockRate) == 0)
            //                {
            //                    Global.Message("Split Data Rate Not Found...Please Check..");
            //                    IntRes = 0;
            //                    btnSave.Enabled = true;
            //                    return;
            //                }
            ...
            //    {
            //        Global.Message("Please Select Invoice No...");
            //        btnSave.Enabled = true;
            //        lueInvoiceNo.Focus();
            //        return;
            //    }
        ...
        private void RBtnType_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (Val.ToString(RBtnType.EditValue) == "B")
                {
                    ...
                    MixSplitProperty objMixSplitProperty = new MixSplitProperty();
                    m_dtbMixDetails = objMixSplit.GetMixStock(0, objMixSplitProperty);
                    dgvMixDetails.Columns["balance_pcs"].Visible = true;
                    dgvMixDetails.Columns["balance_carat"].Visible = true;
                    ...
                    dgvMixDetails.Columns["balance_carat"].VisibleIndex = 4;
                    dgvMixDetails.Columns["trf_pcs"].VisibleIndex = 5;
                    dgvMixDetails.Columns["trf_carat"].VisibleIndex = 6;
                    dgvMixDetails.Columns["trf_rate"].VisibleIndex = 7;
                    dgvMixDetails.Columns["trf_amount"].VisibleIndex = 8;
                    txtCompanyMamoNo.Visible = false;
                    ...
                    {
                        Global.Message("Balance Data Not Found");
                        grdMixDetail.DataSource = null;
                        grdMixDetail.Refresh();
                        dgvMixDetails.BestFitColumns();
                        grdSplitDetail.DataSource = null;
                    ...
                    m_dtbSplitDetails = objMixSplit.GetMixStock_All(0, objMixSplitProperty);
                    ...
                    dgvSplitDetails.Columns["trf_carat"].VisibleIndex = 4;
                ...
                {
                    Global.LOOKUPInvoiceNo(lueInvoiceNo);
                    grdMixDetail.DataSource = null;
                    grdMixDetail.Refresh();
                    dgvMixDetails.BestFitColumns();
                    grdSplitDetail.DataSource = null;
                ...
                if (Val.ToString(RBtnType.EditValue) == "M")
                {
                    dgvMixDetails.Columns["trf_pcs"].Visible = false;
                    dgvMixDetails.Columns["trf_carat"].Visible = false;
                    dgvMixDetails.Columns["trf_rate"].Visible = false;
                    dgvMixDetails.Columns["trf_amount"].Visible = false;
                    grdMixDetail.DataSource = null;
                    grdMixDetail.Refresh();
                    ...
                    grdSplitDetail.Refresh();
                    dgvSplitDetails.BestFitColumns();
                    txtCompanyMamoNo.Visible = true;
                    lblMemo.Visible = true;
                    dgvMixDetails.OptionsBehavior.Editable = false;
                }
            }
            catch (Exception ex)
            {
        ...
        private void txtCompanyMamoNo_KeyDown(object sender, KeyEventArgs e)
        {
            try
            {
                ...
                {
                    m_dtbCompanyMemoIssueDetail = objCompanyMamoIssueReceipt.GetCompanyMemoCarat(Val.ToString(""), Val.ToString(txtCompanyMamoNo.Text));

                    if (Val.ToDecimal(m_dtbCompanyMemoIssueDetail.Rows[0]["outstanding_carat"]) > 0)
                    {
                        m_dtbCompanyMemoIssueDetail.Rows.Clear();

                        m_dtbCompanyMemoIssueDetail = objMixSplit.GetCompanyMemoStock(Val.ToString(txtCompanyMamoNo.Text));
                        dgvMixDetails.Columns["balance_pcs"].Visible = false;
                        dgvMixDetails.Columns["balance_carat"].Visible = false;
                        dgvMixDetails.Columns["rej_pcs"].Visible = true;
                        dgvMixDetails.Columns["rej_carat"].Visible = true;
                        ...
                        dgvMixDetails.Columns["rej_carat"].VisibleIndex = 4;


                        if (m_dtbCompanyMemoIssueDetail.Select("rej_carat > 0").Length > 0)
                        {
                            Party_Memo_No = m_dtbCompanyMemoIssueDetail.Rows[0]["party_memo_no"].ToString();
                            Company_Memo_No = m_dtbCompanyMemoIssueDetail.Rows[0]["company_memo_no"].ToString();
                            grdMixDetail.DataSource = m_dtbCompanyMemoIssueDetail.Select("rej_carat > 0").CopyToDataTable();
                            grdMixDetail.Refresh();
                            dgvMixDetails.BestFitColumns();
                        }
                        ...
                        m_dtbCompanyMemoIssueDetail.Rows.Clear();
                        MixSplitProperty objMixSplitProperty = new MixSplitProperty();
                        m_dtbSplitDetails = objMixSplit.GetMixStock(0, objMixSplitProperty);
                        dgvSplitDetails.Columns["trf_pcs"].Visible = true;
                        dgvSplitDetails.Columns["trf_carat"].Visible = true;
                        dgvSplitDetails.Columns["assort_name"].VisibleIndex = 1;
                        dgvSplitDetails.Columns["sieve_name"].VisibleIndex = 2;
                        ...
                        //grdMixDetail.Enabled = false;
                        dgvMixDetails.OptionsBehavior.Editable = false;
                    }
                    else if (m_dtbCompanyMemoIssueDetail.Rows[0]["outstanding_carat"].ToString() == "")
                    {
                        Global.Message("This Company Memo Number Data Not Exists : " + Val.ToString(txtCompanyMamoNo.Text));
                        grdMixDetail.DataSource = null;
                        grdMixDetail.Refresh();
                        dgvMixDetails.BestFitColumns();
                        grdSplitDetail.DataSource = null;
                    ...
                    else if (Val.ToDecimal(m_dtbCompanyMemoIssueDetail.Rows[0]["outstanding_carat"]) == 0)
                    {
                        Global.Message("This Company Memo Number already Receive : " + Val.ToString(txtCompanyMamoNo.Text));
                        grdMixDetail.DataSource = null;
                        grdMixDetail.Refresh();
                        dgvMixDetails.BestFitColumns();
        ...
        private void txtSlipNo_Validated(object sender, EventArgs e)
        {
            ...
            if (Val.ToString(txtSlipNo.Text) != "")
            {
                DtSlip = objMixSplit.GetSlipNo(Val.ToString(txtSlipNo.Text));
                if (DtSlip.Rows.Count > 0)
                {
                    Global.Message("Slip No already exist.");
                    return;
                }
            }
        ...
        private void backgroundWorker_MixSplit_DoWork(object sender, System.ComponentModel.DoWorkEventArgs e)
        {
                ...
                try
                {
                    ...
                    if (DTab_Split.Select("trf_carat > 0").Length > 0)
                    {
                        ...
                        for (int i = 0; i < DTab_Split.Rows.Count; i++)
                        {
                            ...
                            IntRes = objMixSplit.Save_New(objMixSplitProperty, DLL.GlobalDec.EnumTran.Continue, Conn);
                        }
                    }
                    else
                    {
            ...
            //                    if (Val.ToDecimal(p_numFromStockRate) == 0)
            //                    {
            //                        Global.Message("Rate not found in Master . Assort = " + DTab_Mix.Rows[i]["assort_name"] + " and  Sieve = " + DTab_Mix.Rows[i]["sieve_name"]);
            //                        objMixSplitProperty = null;
            //                        return;
            //                    }

            //                    objMixSplitProperty.from_rate = Val.ToDecimal(p_numFromStockRate);
            //                    objMixSplitProperty.from_amount = Math.Round(Val.ToDecimal(DTab_Mix.Rows[i]["trf_carat"]) * Val.ToDecimal(p_numFromStockRate), 3);
            //                    objMixSplitProperty.to_assort_id = Val.ToInt(416);
            //                    objMixSplitProperty.to_sieve_id = Val.ToInt(DTab_Mix.Rows[i]["sieve_id"]);
            //                    objMixSplitProperty.to_sub_sieve_id = Val.ToInt(387);
            //                    objMixSplitProperty.to_pcs = Val.ToInt(0);
            ...
            //                    objMixSplitProperty.to_amount = Math.Round(Val.ToDecimal(DTab_Split.Rows[i]["trf_carat"]) * Val.ToDecimal(p_numToStockRate), 3);
            //                    objMixSplitProperty.mixsplit_type_id = Val.ToInt(2);
            //                    objMixSplitProperty.form_id = m_numForm_id;
            //                    objMixSplitProperty.transaction_type_id = Val.ToInt(2);
            //                    objMixSplitProperty.currency_id = Val.ToInt(GlobalDec.gEmployeeProperty.currency_id);
                        ...
                        if (DTab_Mix.Rows.Count == 1)
                        {
                            ...
                            for (int i = 0; i < DTab_Mix.Rows.Count; i++)
                            {
                                for (int j = 0; j < DTab_Split.Rows.Count; j++)
                                {
                                    ...
                                    objMixSplitProperty.from_invoice_id = Val.ToInt64(lueInvoiceNo.EditValue);
                                    ...
                                    if (count == 0)
                                    {
                                        objMixSplitProperty = objMixSplit.Save_Mix(objMixSplitProperty, DLL.GlobalDec.EnumTran.Continue, Conn);
                                        count = 1;
                                    }
                                    else
                                    {
                                        objMixSplitProperty.count = count;
                                        objMixSplitProperty = objMixSplit.Save_Split(objMixSplitProperty, DLL.GlobalDec.EnumTran.Continue, Conn);
                                    }
                                    //objMixSplitProperty = objMixSplit.Save(objMixSplitProperty, DLL.GlobalDec.EnumTran.Continue, Conn);
        ...
        private void btnUpdate_Click(object sender, EventArgs e)
        {
            ...
            int IntRes = objMixSplit.Update_Invoice_No(objMixSplitProperty);

            if (IntRes == -1)
            {
                Global.Confirm("Error in Update Invoice No");
            }
            else
            {
                Global.Confirm("Invoice No Update Successfully");
