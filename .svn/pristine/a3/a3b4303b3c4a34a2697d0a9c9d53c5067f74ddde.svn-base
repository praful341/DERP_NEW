...
{
    public partial class FrmMstBillManagerMaster : DevExpress.XtraEditors.XtraForm
    {
        #region Data Member
        ...
        MstBillManagerMaster ObjStoreManager = new MstBillManagerMaster();

        #endregion

        #region Constructor

        public FrmMstBillManagerMaster()
        {
            InitializeComponent();
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            MstBillManagerMasterProperty BillManagerMasterP = new MstBillManagerMasterProperty();

            BillManagerMasterP.manager_id = Val.ToInt64(lblMode.Tag);
            BillManagerMasterP.manager_name = txtManagerName.Text;
            BillManagerMasterP.mobile_no = Val.ToString(txtMobile.Text);
            BillManagerMasterP.address = Val.ToString(txtAddress.Text);
            BillManagerMasterP.city_id = Val.ToInt64(lueCity.EditValue);
            BillManagerMasterP.state_id = Val.ToInt64(lueState.EditValue);
            BillManagerMasterP.company_id = Val.ToInt64(lueCompany.EditValue);
            BillManagerMasterP.location_id = Val.ToInt64(lueLocation.EditValue);
            BillManagerMasterP.department_id = Val.ToInt64(lueDepartment.EditValue);

            int IntRes = ObjStoreManager.SaveBillManager(BillManagerMasterP);

            BillManagerMasterP = null;
            if (IntRes != 0)
            {
                if (Val.ToInt(lblMode.Tag) == 0)
                {
                    Global.Confirm("Bill Manager Data Saved Successfully");
                }
                else
                {
                    Global.Confirm("Bill Manager Data Updated Successfully");
                }
